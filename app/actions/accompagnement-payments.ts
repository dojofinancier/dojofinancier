...
const createAccompagnementPaymentIntentSchema = z.object({
...
export async function createAccompagnementEnrollmentAction(data: {
  ...
  try {
    // Idempotency: check if enrollment already exists for this paymentIntentId
    ...
    // Generate order number (continue the existing sequence)
    ...
    if ((error as { code?: string })?.code === "P2002") {
      // Unique constraint — enrollment already exists
      const existing = await prisma.accompagnementEnrollment.findFirst({
        where: { paymentIntentId: data.paymentIntentId },
        select: { id: true },
      });
